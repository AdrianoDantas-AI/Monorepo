...
test("buildTripProgressSnapshot calcula distancias para trip ativa", () => {
  ...
  assert.ok(snapshot.eta_s !== null);
  assert.ok((snapshot.eta_s ?? 0) > 40 && (snapshot.eta_s ?? 0) < 80);
});

test("buildTripProgressSnapshot reduz ETA ao avancar na rota", () => {
  const trip = {
    id: "trip_progress_unit_003",
    tenant_id: "tenant_progress_unit_001",
    vehicle_id: "vehicle_progress_unit_003",
    driver_id: "driver_progress_unit_003",
    status: "active" as const,
    stops: [
      {
        id: "stop_1",
  ...
  const nearStart = buildTripProgressSnapshot(trip, { lat: 0, lng: 0.001 });
  const nearEnd = buildTripProgressSnapshot(trip, { lat: 0, lng: 0.009 });

  assert.ok((nearStart.eta_s ?? 0) > (nearEnd.eta_s ?? 0));
});

test("buildTripProgressSnapshot aplica fallback de velocidade quando total_duration_s invalido", () => {
  const trip = {
    id: "trip_progress_unit_004",
    tenant_id: "tenant_progress_unit_001",
    vehicle_id: "vehicle_progress_unit_004",
    driver_id: "driver_progress_unit_004",
    status: "active" as const,
    stops: [
      {
        id: "stop_1",
